...
const Home = () => {
  ...
  const Category = ({ title, image, id }: any) => {
    const isSelected = selectedCategoryId === id; // Check if category is selected
    const backgroundColor = isSelected ? 'lightgreen' : 'white'; // Set background color based on selection

    return (
      <TouchableOpacity   style={[st.itemCategory, selectedCategoryId === id && st.selectedCategoryId]} onPress={() => setSelectedCategoryId(id)}>
